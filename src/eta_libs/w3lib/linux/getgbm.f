...
Cmp
C	write(6,*) 'calling getgb1s with NLEN= ', NLEN
Cmp
          CALL GETGB1S(CBUF,NLEN,NNUM,JR,JPDS,JGDS,JENS,
     &                 KR,KPDS,KGDS,KENS,LSKIP,LGRIB,IRGS)
Cmp
Cmp	write(6,*) '1-IRGS= ', IRGS
C	write(6,*) 'KR= , MNUM= ', KR,MNUM
...
C	write(6,*) 'returning from GETGIR with NLEN=  ', NLEN
        ENDIF
        IF(IRGI.LE.1) THEN
          CALL GETGB1S(CBUF,NLEN,NNUM,JR,JPDS,JGDS,JENS,
     &                 KR,KPDS,KGDS,KENS,LSKIP,LGRIB,IRGS)
C	write(6,*) 'IRGS,IRGI= ', IRGS,IRGI
          IF(IRGS.EQ.0) K=KR+MNUM
          IF(IRGI.EQ.1.AND.IRGS.EQ.0) MNUM=-1-MNUM
          IF(IRGI.EQ.1.AND.IRGS.GT.0) MNUM=MNUM+NNUM
        ENDIF
