...
C       KFDRIVE  - DRIVER FOR KAIN_FRITSCH CONVECTION
C       KFTEND   - TENDENCIES FROM CONVECTION APPLIED TO ARRAYS
...
C
C     IF(MYPE.EQ.0)CALL W3TAGB('ETAFCST ',0097,0365,0060,'NP22   ')
C
C***
...
C-----------------------------------------------------------------------
 2000 CONTINUE
      ...
      NTSD=NTSD+1
C     IF(MYPE.EQ.0)WRITE(6,2001) NTSD,(NTSD-1)*DT
...
C
      if(mype.eq.0)then
        ...
        write(6,1099)'   DPTE      ',pdte_tim*1.e-3,pct
...
C
      IF(MYPE.EQ.0)CALL W3TAGE('ETAFCST ')
C
