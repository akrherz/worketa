...
C-----------------------------------------------------------------------
                              L O G I C A L
     & RUN,RUNB,FIRST,RESTRT,SIGMA,EXBC,NEST,HYDRO,SPLINE
     &,INSIDEH,INSIDEV
C-----------------------------------------------------------------------
...
C***
      ...
      IF(SINGLRST)THEN
         print *,"SINGLRST"
         print *,"CALLING READ_RESTRT"
        CALL READ_RESTRT
      ELSE
        print *,"CALLING READ_RESTRT2"
        CALL READ_RESTRT2
      ENDIF
      res_tim=timef()-btim
C
...
C-------------------------------------------------------------
      IF(MYPE.EQ.0)THEN
Cmp
        open(unit=NBC,file='bndy.file',access='sequential',
     +          form='unformatted')
Cmp
        REWIND NBC
...
C
      ...
      ENDDO
C
      CALL W3DOXDAT(IBCDAT,IBCDOW,IBCDOY,JBCDAY)
      CALL W3DOXDAT(INIDAT,INDOW,INDOY,JMDDAY)
      JBCHR  = JBCDAY*24 + IHRSTB
      JMDHR  = JMDDAY*24 + IHRST
      JDIFHR = JMDHR     - JBCHR
      IF(MYPE.EQ.0)WRITE(LIST,*)'INIT:  READ BOUNDARY CONDITIONS'
C
      IF(JDIFHR.NE.0)THEN
        ITBOCO=NINT(TBOCO)
        NPOSIT=JDIFHR/ITBOCO+1
C
        IF(MYPE.EQ.0)THEN
          WRITE(LIST,*)' LOOP NPOSIT=',NPOSIT,' TIMES TO READ BC'
        ENDIF
C
        IF(RESTRT)THEN
          IF(MYPE.EQ.0)THEN
            WRITE(LIST,*)'  ',NPOSIT,' DUMMY READS INTO NBC=',
     1                   NBC,'.  USE BC IN IC.'
          ENDIF
        ELSE
          IF(MYPE.EQ.0)THEN
            WRITE(LIST,*)'INIT:  ',NPOSIT,' READS INTO NBC=',
     1                   NBC,' FOR BC'
          ENDIF
        ENDIF
        DO NRD=1,NPOSIT
...
C-------------------------------------------------------------
      IF(MYPE.EQ.0)THEN
C       WRITE(LIST,*)'INIT:  READ IOUT,NSHDE,NTSD=',IOUT,NSHDE,NTSD
      ENDIF
C
      DO I=1,NMAP
         IOUT=I
...
C
           NCLDCK=NINT(600./DT)
           PRINT*,'NCLDCK 1 ', NCLDCK
           NCLDCK=MAX0(NCLDCK,1)
           PRINT*,'NCLDCK 2 ', NCLDCK
           TSTKF=REAL(NCLDCK)

        END IF   ! END OF KF MODIFICATIONS
C     
...
C
C-------------------------------------------------------------------
C***  SOME INITAL VALUES RELATED TO TURBULENCE SCHEME
C-------------------------------------------------------------------
C
      DO J=JS_LOC_TABLE(MYPE),JE_LOC_TABLE(MYPE)
      DO I=IS_LOC_TABLE(MYPE),IE_LOC_TABLE(MYPE)
