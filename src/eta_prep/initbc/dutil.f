...
c
            ...
            elseif (gproj .eq. 'LC') then
	    	call lcc_ij(aphd,almd,x,y,gds)
C            elseif (gproj .eq. 'CE') then
C               call latlon_2_coneqij(1,aphd,almd,x,y)
            elseif (gproj .eq. 'LL') then
	    	call ced_ij(aphd,almd,x,y,gds)
            endif
...
c
      ...
      character*2   gproj
c
      common/sectorsize/nxsec,nysec,nz,ip,jp
c
...
c===============================================================================
c
      subroutine getdata(fname,pr,uw,vw,ht,mr,sfcgrid)
c
...
Cmp(2000/04/18 	eliminate header information)
Cmp
Cmp	backing away from this strategy at least temporarily
C
...
C      read(1) (((th(i,j,k),i=1,nx),j=1,ny),k=nz,1,-1)
      read(1) (((ht(i,j,k),i=1,nx),j=1,ny),k=nz,1,-1)
      read(1) (((mr(i,j,k),i=1,nx),j=1,ny),k=nz,1,-1)
C      read(1) (((ex(i,j,k),i=1,nx),j=1,ny),k=nz,1,-1)

	do K=1,nz
	do J=1,ny
	do I=1,nx
	if (mr(i,j,k) .gt. 1.) write(6,*) 'Q in getdata: ', 
     +		i,j,k,mr(i,j,k)
	enddo
	enddo
	enddo
	...
	write(6,*) 'U,V,Z,Q : ', uw(nx/2,ny/2,5),vw(nx/2,ny/2,5),
     +			ht(nx/2,ny/2,5),mr(nx/2,ny/2,5)
	read(1) (pr(k),k=nz,1,-1)
	
	
C	write(6,*) 'pr values ', pr

Cmp
C	write(6,*) 'nsfcfld= ', nsfcfld
C	write(6,*) 'to sfcgrid read'
        read(1) sfcgrid

C	write(6,*) 'past sfcgrid read'
      close(1)
c
      do k=1,nz
	pr(k)=pr(k)*100.
C	write(6,*) 'pressure : ', k, pr(k)
      enddo
c
      return
