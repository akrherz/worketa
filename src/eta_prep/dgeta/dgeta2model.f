...
c
      implicit none
c
Cansi      integer*4 nx,ny,nz          !Degrib grid dimensions
      integer nx,ny,nz          !Degrib grid dimensions
c
Cansi      character*255 degrib_dir,degrib_file,outdir,gribfile
      character(LEN=255):: degrib_dir,degrib_file,outdir,gribfile
c
Cansi      real*4 esat,es
Cansi      integer*4 datsav,n,l,kpds(200)	
      real esat,es
...
c
      implicit none
c
Cansi      real*4    cp,kappa
      real    cp,kappa
      parameter(cp=1004.,kappa=287./1004.)
c
Cansi      integer*4 nx,ny,nz,i,j,k,l,n,it,ip,jp,nsfcfld
      integer nx,ny,nz,i,j,k,l,n,it,ip,jp,nsfcfld
     .         ,iyear,imonth,iday,ifcsthr
     .	       ,datsav,ii,jj
      integer kgds(200)

Cansi	real*4 crot(nx*ny),srot(nx*ny)
	real crot(nx*ny),srot(nx*ny),tbar,old,fact,biassum
	real z1000t,z975t
c
Cansi      real*4 ht(nx,ny,nz)              !Isobaric heights (m)
...
c
Cansi  character*255 degrib_dir,degrib_file,outdir,outfile,gribfile
Cansi      character*255 gdsfile
Cansi      character*2   gproj
Cansi      character*11   atime
Cansi      character*6   model
...
Cmp
	...
	if (nz .ge. 38) then
      do k=1,nz
Cmp         pr(k)=1000.-float(k*25)
         pr(k)=1025.-float(k*25)
      enddo
	elseif (nz .eq. 20) then
C
C	001219  HELLO!  104 data still using 1000 mb ?????
C
	do k=1,nz
	pr(k)=1050-float(k*50)
	enddo
	endif
c 
...
Cmp     moisture every 50 for grid 104...handle in special way
	...
	write(6,*) 'copying value from ', pr(K+1)
...
C
C	DO AN INITIAL SMOOTHING OF HEIGHT FIELD?
C

C	call smooth(ht,nx,ny,nz,5)
...
C	write(6,*) 'I,J, 1000T, 975T, applied tbar: ', 
C     +			I,J,z1000t,z975t,tbar
Ctst	ht(i,j,1)=ht(i,j,2)-fact*tbar
	ht(i,j,1)=ht(i,j,2)-fact*tbar
	biassum=biassum+1
	endif
	
C	biassum=biassum+(ht(i,j,1)-old)

C	if (I .eq. 123 .and. J .eq. 37) then
C 	write(6,*) 'tp(i,j,1),tbar: ', tp(i,j,1),tbar
C	write(6,*) 'old,new Z1000s: ', old,ht(i,j,1)
C	write(6,*) '5 lowest heights: ',ht(i,j,1),ht(i,j,2),ht(i,j,3),
C     +					ht(i,j,4),ht(i,j,5)
C	write(6,*) '5 lowest temps: ', tp(i,j,1),tp(i,j,2),tp(i,j,3),
C     +					tp(i,j,4),tp(i,j,5)
...
C	write(6,*) '1000 Z'
	do JJ=40,34,-1
C	write(6,633) (ht(II,JJ,1),II=120,126)
	enddo

C	write(6,*) '975 Z'
	do JJ=40,34,-1
C	write(6,633) (ht(II,JJ,2),II=120,126)
	enddo
...
Cansi  743   format(30(e9.3,x))
Cansi  744   format(30(f7.1,x))
...
c
Cmp
Cmp   some of this header information is projection dependent
Cmp	
      ip=1
      jp=1
...
C
      nsfcfld=12
C
C	if (nx .eq. 349 .or. nx .eq. 185) gproj='LC'
C	if (nx .eq. 147) gproj='PS'
	...
	IF (gproj .eq. 'PS') THEN
C
C	lat1=true lat, lat2=center lon, lon0=rotation
C
	lat1=60.
	lat2=datsav(5)/1000.
	lon0=0.
...
Cmp     lambert subroutine to calculate the ne values
...
C end new stuff
	...
	if (gproj .eq. 'LC') then
	write(6,*) 'calling lambert'
	call lambert (urlat,urlon)
	write(6,*) 'back from lambert'
	elseif (gproj .eq. 'PS') then
C
C	called just to get urlat, urlon here
C
	...
	write(6,*) 'LL corner points ', sw(1),sw(2)
	write(6,*) 'UR corner points ', ne(1),ne(2)
	write(6,*) 'dimensions nx,ny,nz ', nx,ny,nz
	if (gproj .eq. 'LC') then
	write(6,*) 'lats, clon ', lat1,lat2,lon0
	elseif (gproj .eq. 'PS') then
	write(6,*) 'lat,clon ', lat1,lat2
	write(6,*) '********************************************'
	endif

Cmp	what is written below is projection dependent!
Cmp
Cmp  used the same name for different items (lat1,lat2,lon0)
cmp
...
c
      implicit none
c
Cansi      integer*4 nxny,nz,i,datsav
      integer nxny,nz,i,datsav
c
Cansi      real*4 pr(nz),ht(nxny,nz),tp(nxny,nz)
...
C	rotate the winds at this point
	...
	write(6,*) 'dont recognize projection for wind rotation...'
	write(6,*) 'STOPPING!!!!!!!!!!!'
	stop
	endif
...
c
      implicit none
c
Cansi      integer*4 nx,ny,nz,i,j,k,kk
      integer nx,ny,nz,i,j,k,kk
c
Cansi      real*4 rh(nx,ny,nz),pr(nz)
...
Cansi        integer*4 kgds(200),kpds(200),len,kerr
...
C	write(6,69) I,J,RLAT(N),RLON(N)
	enddo
Cansi   69	format(I3,x,I3,x,2(f7.3,x))
