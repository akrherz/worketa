...
!
      ...
      subroutine rtllold(tlmd,tphd,tlm0d,ctph0,stph0,almd,aphd)
c
c *** Rotate eta lat, lon (tphd, tlmd) to geographical lat, lon (aphd, almd).
c     All calculations are done in degrees.
c
      implicit none
c
      real*4 tlmd,tphd 
...
c
      ...
      stlm=sind(tlmd)
c
      aphd=asind(stph0*ctph*ctlm+ctph0*stph)
c
      cph=cosd(aphd)
      almd=tlm0d+asind(stlm*ctph/cph)
c
      return
      end
